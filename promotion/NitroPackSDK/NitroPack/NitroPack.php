...
class NitroPack {
    ...
    public function __construct($siteId, $siteSecret, $userAgent = NULL, $url = NULL, $dataDir = __DIR__) {
            ...
            if ($queryStr) {
                ...
                if ($queryParams) {
                    foreach ($this->config->IgnoredParams as $ignorePattern) {
                        $regex = "/^" . $this->wildcardToRegex($ignorePattern) . "$/";
        ...
        $this->pageCache = new Pagecache($this->url, $this->userAgent, self::getCookies(), $this->config->PageCache->SupportedCookies, $this->isAJAXRequest());
    ...
    public function hasLocalCache() {
        if (!$this->isAllowedUrl($this->url) || !$this->isAllowedRequest()) return false;
        return $this->pageCache->hasCache() && !$this->pageCache->hasExpired($this->config->PageCache->ExpireTime);
    }

    public function hasRemoteCache($layout) {
        if (!$this->isAllowedUrl($this->url) || !$this->isAllowedRequest() || $this->isPageCacheLocked()) return false;
        $resp = $this->api->getCache($this->url, $this->userAgent, self::getCookies(), false, $layout);
    ...
    public function purgeCache($url = NULL, $tag = NULL, $purgeType = PurgeType::COMPLETE, $reason = NULL) {
        ...
        try {
            ...
            if ($url || $tag) {
                ...
                if ($tag) {
                    $urls = $this->api->getTaggedUrls($tag);
                    while($urls) {
                        foreach ($urls as $url) {
                            if ($invalidate) {
                                $localResult &= $this->invalidateLocalUrlCache($url["url"]);
                            } else {
                                $localResult &= $this->purgeLocalUrlCache($url["url"]);
                            }
                        }

                        $apiResult &= $this->api->purgeCache(array_map(function($url) { return $url["url"]; }, $urls), false, $reason);
                        $urls = $this->api->getTaggedUrls($tag);
                    }
                }
            } else {
        ...
        if ($this->config->EnabledURLs->Status) {
            if (!empty($this->config->EnabledURLs->URLs)) {
                foreach ($this->config->EnabledURLs->URLs as $enabledUrl) {
                    ...
                    if (preg_match('~^' . str_replace(array('~'), array('\~'), $this->wildcardToRegex($enabledUrlModified)) . '$~', $url)) {
                        return true;
                    }
                }
            ...
            if (!empty($this->config->DisabledURLs->URLs)) {
                foreach ($this->config->DisabledURLs->URLs as $disabledUrl) {
                    ...
                    if (preg_match('~^' . str_replace(array('~'), array('\~'), $this->wildcardToRegex($disabledUrlModified)) . '$~', $url)) {
    ...
    public function isAllowedRequest($allowServiceRequests = false) {
        if ($this->isAJAXRequest() || !$this->isGetRequest()) {// TODO: Allow URLs which match a pattern in the AJAX URL whitelist
        ...
        foreach ($knownDeviceTypes as $deviceType) {
            ...
            Filesystem::dirForeach($deviceTypeDir, function($urlDir) {
                if (substr($urlDir, -2) !== "_i") {
                    Filesystem::rename($urlDir, $urlDir . "_i");
                }
            });
        }
        return true;
    ...
    private function wildcardToRegex($str) {
        return implode(".*?", array_map("preg_quote", explode("*", $str)));
    }
}
