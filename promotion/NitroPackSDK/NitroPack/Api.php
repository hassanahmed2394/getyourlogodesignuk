...
class Api {
    ...
    private $tagger;
    private $url;
    private $allowedWebhooks = array('config', 'cache_clear', 'sitemap');
    ...
    public function getUrls($page = 1, $limit = 250, $search = NULL, $deviceType = NULL) {
        return $this->url->get($page, $limit, $search, $deviceType);
    }

    public function getUrlsCount($search = NULL, $deviceType = NULL) {
        $resp = $this->url->count($search, $deviceType);
    ...
    public function runWarmup($urls = NULL) {
        return $this->warmup->run($urls);
    }
